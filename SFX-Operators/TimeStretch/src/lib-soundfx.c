/****h* TimeStretch/lib-soundfx.c [4.3] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
*    $VER: lib-soundfx.c 4.3 (17.02.04) © by Stefan Kost 1998-2004
*  FUNCTION
*    TimeStretch Operator module for SoundFX
*  AUTHOR
...
*    19.May.2003	V 1.20	initial version
*  NOTES
*
*  IDEAS
*    * Dynamic-correction
*      * generate an volume-envelope of the source (not that easy in this case)
*        * we need the envelope at the writepos and readpos
*        * so have another ringbuffer, where we put the envelopedata into
...
#define GADID_WINSIZE	(GADID_MOD1+MOD_IDCT)
#define GADIX_WINSIZE	(GADIX_MOD1+MOD_IXCT)

#define GADID_SMOOTH	(GADID_WINSIZE+1)
#define GADIX_SMOOTH	(GADIX_WINSIZE+1)

#define GADID_DYNCOR	(GADID_SMOOTH+1)
#define GADIX_DYNCOR	(GADIX_SMOOTH+1)

#define GADID_CHANLINK	(GADID_DYNCOR+1)
#define GADIX_CHANLINK	(GADIX_DYNCOR+1)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		ng.ng_GadgetText	="DynCor";
		ng.ng_GadgetID		=GADID_DYNCOR;
		g=CreateGadget(CHECKBOX_KIND,g,&ng,GTCB_Checked,instance->dyncor,GTCB_Scaled,TRUE,TAG_DONE);
		modGadgets[GADIX_DYNCOR]=g;
		...
		if(g) {
			...
			DrawTitle(modrp,"TimeStretch Factor" ,xs1,ys2,STDFRM_WIDTH);
			...
			DrawGadWB(modrp,xs1,ys2,xh,MODSEL_HEIGHT);					/* TimeStretch */
								...
								&& !HandleCfgSel      (&cfgsel       ,modWin,modGadgets,GADID_CFGSEL,GADIX_CFGSEL,gnr,icode,isec,imic)) {
									switch(gnr) {
										...
										case GADID_SMOOTH:
											...
											break;
										case GADID_DYNCOR:
											instance->dyncor=((modGadgets[GADIX_DYNCOR]->Flags&GFLG_SELECTED)==GFLG_SELECTED);
											break;
										case GADID_CHANLINK:
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	UC_ConvertParameter(param,instance->mod1.vals,UGRP_RELTIME,RTME_FC);
	instance->timefs=atof(param);
	UC_ConvertParameter(param,instance->mod1.vale,UGRP_RELTIME,RTME_FC);
	instance->timefe=atof(param);
	instance->timefd=instance->timefe-instance->timefs;
	// DEBUG
	MSG2("time factor : %6.4lf ... %6.4lf",instance->timefs,instance->timefe);
	// DEBUG
		...
		if(activate((SFXCoreInstance *)instance)) {
			if(!dstbuf) {
				if(dstbuf=NewSample()) {
					...
					dstbuf->slen=(srcbuf->slen-rm_len)+instance->rlen;
					dstbuf->zoomxl=dstbuf->slen;
					if(AllocSampleMem(dstbuf,dstbuf->slen,CH_COUNT(srcbuf->channels))) {
						newdst=TRUE;
					}
					else { RemSample(dstbuf);deactivate((SFXCoreInstance *)instance);Message(errAllocateMemory,NULL,__FILE__,__LINE__);return(FALSE); }
				}
				else { Message(errAddSampleBuffer,NULL,__FILE__,__LINE__);return(FALSE); }
			}
			MSG2("## slen %ld -> rlen %ld",instance->slen,instance->rlen);
				...
				if(IS_CHANNEL_ACTIVE(l,srcbuf->channelmask)) {
					...
					if(srcsa && dstsa) {
						...
						if(instance->chanlink) {
							if(instance->dyncor) {
								MSG("10 dynamic correction is on, seeking is off");
								for(srcsa->seg=dstsa->seg=0;((srcsa->seg<=srcsa->seganz) && (instance->curlen<instance->slen));) {
									...
									//MSG4("curlen=%6ld/%6ld  rptr1=%9.3lf  wptr1=%6ld",(instance->curlen-instance->winsize),instance->curlen,instance->rptr1,instance->wptr1);
									process_noseek_dyncor((SFXCoreInstance *)instance,blen);
								...
								MSG("00 dynamic correction is off, seeking is off");
								for(srcsa->seg=dstsa->seg=0;((srcsa->seg<=srcsa->seganz) && (instance->curlen<instance->slen));) {
									if(instance->spos==0) {
										instance->sbuf=(SAMPLE *)srcsa->buffer1;			// Pointer auf src-Block holen
										instance->spos=blen;
							...
							if(instance->dyncor) {
								MSG("11 dynamic correction is on, seeking is on");
								for(srcsa->seg=dstsa->seg=0;((srcsa->seg<=srcsa->seganz) && (instance->curlen<instance->slen));) {
									if(instance->spos==0) {
										instance->sbuf=(SAMPLE *)srcsa->buffer1;			// Pointer auf src-Block holen
										instance->spos=blen;
									...
									//MSG4("curlen=%6ld/%6ld  rptr1=%9.3lf  wptr1=%6ld",(instance->curlen-instance->winsize),instance->curlen,instance->rptr1,instance->wptr1);
									process_seek_dyncor((SFXCoreInstance *)instance,blen);
									if(instance->dpos==blen) {
										SAWrite(dstsa,dstsa->blen);dstsa->seg++;			// akt Block schreiben
										instance->dbuf=(SAMPLE *)dstsa->buffer1;			// Pointer auf dst-Block
										instance->dpos=0;
								...
								MSG("01 dynamic correction is off, seeking is on");
								for(srcsa->seg=dstsa->seg=0;((srcsa->seg<=srcsa->seganz) && (instance->curlen<instance->slen));) {
									if(instance->spos==0) {
										instance->sbuf=(SAMPLE *)srcsa->buffer1;			// Pointer auf src-Block holen
										instance->spos=blen;
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)){ 
	...
	if(!stricmp(RexxPar0,"SFX_SetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"DynCor"))  	{ instance->dyncor	=atoi(RexxPar3)&0x1;pf=TRUE; }
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"DynCor"))		{ sprintf(ret,"%d",instance->dyncor);pf=TRUE; }
...
void LoadSettings(struct Instance *instance,char *fname){ 
	...
	instance->dyncor	=CfgIO_ReadBool	   (cfg,"General"  ,"DynCor"  ,FALSE);
...
void SaveSettings(struct Instance *instance,char *fname){ 
	...
	CfgIO_WriteBool			  (cfg,"General"    ,"DynCor"  ,instance->dyncor);
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod1.vals,"1.5",UGRP_RELTIME,RTME_FC);
	UC_ConvertParameter(instance->mod1.vale,"1.5",UGRP_RELTIME,RTME_FC);
