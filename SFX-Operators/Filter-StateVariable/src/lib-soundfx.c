/****h* Filter-StateVariable/lib-soundfx.c [4.2] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
*    $VER: lib-soundfx.c 4.2 (26.02.02) © by Stefan Kost 1998-2002
*  FUNCTION
*    Filter-StateVariable Operator module for SoundFX
*  AUTHOR
*    Stefan Kost			sk		kost@imn.htwk-leipzig.de
*  CREATION DATE
*    18.Jul.1998
*  MODIFICATION HISTORY
*    26.Feb.2002	V 4.2	most recent version
...
#define LAST_GADIX		(GADIX_TYPE+1)
...
STRPTR TypeLabels[]={ "LowPass","HighPass","BandPass","BandStop",0l };
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		ng.ng_LeftEdge		=xs1+RunTime->scx[53];				/* Par */
		...
		if(g) {
			...
			DrawGadWB(modrp,xs1,ys3,xh,MODSEL_HEIGHT);					/* Cut-Off */
			DrawGadWB(modrp,xs1,ys4,xh,MODSEL_HEIGHT);					/* Resonance */
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	if(UC_GetParameterUnit(instance->mod2.vals,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		...
		instance->frqs=atof(param)/((double)srcbuf->srat);
		//instance->frqs=atof(param)/((double)srcbuf->srat/2.0);
	}
	else {
	...
	if(instance->frqs>1.0) instance->frqs=1.0;
	if(UC_GetParameterUnit(instance->mod2.vale,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		...
		instance->frqe=atof(param)/((double)srcbuf->srat);
		//instance->frqe=atof(param)/((double)srcbuf->srat/2.0);
	}
	else {
	...
	if(instance->frqe>1.0) instance->frqe=1.0;
	instance->frqd=instance->frqe-instance->frqs;
	MSG3("frq = %lf...%lf , d=%lf",instance->frqs,instance->frqe,instance->frqd);
	...
	MSG3("res = %lf...%lf , d=%lf",instance->ress,instance->rese,instance->resd);
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod2.vals,"0.5",UGRP_RELFREQUENCY,RFRQ_FC);
	UC_ConvertParameter(instance->mod2.vale,"0.5",UGRP_RELFREQUENCY,RFRQ_FC);
	instance->mod2.bshape=BSHP_NONE;
	instance->mod2.desc[0]='\0';

	UC_ConvertParameter(instance->mod3.vals,"5.0",UGRP_PROPOTION,PROP_FC);
	UC_ConvertParameter(instance->mod3.vale,"5.0",UGRP_PROPOTION,PROP_FC);
