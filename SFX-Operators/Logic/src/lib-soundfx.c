...
*  FUNCTION
*    Logic Operator module for SoundFX
*  AUTHOR
...
#define GADID_TYP		(GADID_MOD2+MOD_IDCT)
#define GADIX_TYP		(GADIX_MOD2+MOD_IXCT)

#define LAST_GADIX		(GADIX_TYP+1)
...
UBYTE *TypeLabels[]={"False","A And B","A > B","A","A < B","B","A Xor B","A Or B","A Nor B","E Equ B","Not B","A >= B","Not A","A <= B","A Nand B","True",0l};
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		ng.ng_GadgetID		=GADID_TYP;
		ng.ng_Flags			=PLACETEXT_LEFT;
		g=CreateGadget(CYCLE_KIND,g,&ng,GTCY_Labels,TypeLabels,GTCY_Active,instance->typ,TAG_DONE);
		modGadgets[GADIX_TYP]=g;
		...
		if(g) {
			...
			DrawTitle(modrp,"Source (A)"		,xs1,ys1,STDFRM_WIDTH);
			DrawTitle(modrp,"Effect"			,xs1,ys2,STDFRM_WIDTH);
			DrawTitle(modrp,"Logic Operand (B)"	,xs1,ys3,STDFRM_WIDTH);
			DrawTitle(modrp,"Logic Function"	,xs1,ys4,STDFRM_WIDTH);
			...
			DrawGadWB(modrp,xs1,ys3,xh,MODSEL_HEIGHT);					/* Log Op. */
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	instance->logs=atof(param);
	UC_ConvertParameter(param,instance->mod2.vale,UGRP_AMPLITUDE,AMP_LEVEL);
	instance->loge=atof(param);
	instance->logd=instance->loge-instance->logs;
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!stricmp(RexxPar0,"SFX_SetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Type"))			{ instance->typ=CfgIO_MatchString(RexxPar3,TypeLabels);pf=TRUE; }
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Type"))			{ strcpy(ret,TypeLabels[instance->typ]);/*pf=TRUE;*/ }
	}
...
void LoadSettings(struct Instance *instance,char *fname) {
	...
	instance->typ=	CfgIO_MatchString(
					CfgIO_ReadString		(cfg,"General"	,"Type"	,TypeLabels[instance->typ]),TypeLabels);
}

void SaveSettings(struct Instance *instance,char *fname) {
	SaveModSettings(cfg,instance->moddesc);
	CfgIO_WriteString		  (cfg,"General"	,"Type"		,TypeLabels[instance->typ]);
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod2.vals,"20000 lv",UGRP_AMPLITUDE,AMP_LEVEL);
	UC_ConvertParameter(instance->mod2.vale,"20000 lv",UGRP_AMPLITUDE,AMP_LEVEL);
...
void RefreshGUI(struct Instance *instance,struct Gadget *modGadgets[],struct Window *modWin) {
	...
	SetModulator(&(instance->mod1),modWin,modGadgets,GADIX_MOD1);
	SetModulator(&(instance->mod2),modWin,modGadgets,GADIX_MOD2);
	GT_SetGadgetAttrs(modGadgets[GADIX_TYP],modWin,0l,GTCY_Active,instance->typ,TAG_DONE);
}
