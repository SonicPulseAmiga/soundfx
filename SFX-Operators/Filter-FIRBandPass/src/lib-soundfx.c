/****h* Filter-FIRBandPass/lib-soundfx.c [4.2] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
...
*  FUNCTION
*    Filter-FIRBandPass Operator module for SoundFX
*  AUTHOR
...
#define FILTER_FIRBANDPASS_SOUNDFX_C
...
#define GADID_ANZ		(GADID_MOD3+MOD_IDCT)
#define GADIX_ANZ		(GADIX_MOD3+MOD_IXCT)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		g=AddWinFunction(&(instance->wfkt1),modGadgets,g,GADID_WFKT,GADIX_WFKT,xs1,ys5+RunTime->scy[FRMCONTENT_YOFFSET]);
		...
		if(g) {
			...
			RText(modrp,"Window",xs1+RunTime->scx[45],ys5+RunTime->scy[FRMTEXT_YPOS(1)]);

			DrawTitle(modrp,"Source"			,xs1,ys1,STDFRM_WIDTH);
			DrawTitle(modrp,"Effect"			,xs1,ys2,STDFRM_WIDTH);
			DrawTitle(modrp,"Low Cut-Off"		,xs1,ys3,STDFRM_WIDTH);
			DrawTitle(modrp,"High Cut-Off"		,xs1,ys4,STDFRM_WIDTH);
			DrawTitle(modrp,"Filter-Parameter"	,xs1,ys5,STDFRM_WIDTH);
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	if(UC_GetParameterUnit(instance->mod2.vale,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		...
		instance->frq1e=(atof(param)*2.0)/(srcbuf->srat);
	}
	else {
		UC_ConvertParameter(param,instance->mod2.vale,UGRP_RELFREQUENCY,RFRQ_FC);
		instance->frq1e=atof(param);
	}
	instance->frq1d=instance->frq1e-instance->frq1s;

	if(UC_GetParameterUnit(instance->mod3.vals,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		UC_ConvertParameter(param,instance->mod3.vals,UGRP_ABSFREQUENCY,AFRQ_HZ);
		instance->frq2s=(atof(param)*2.0)/(srcbuf->srat);
	}
	else {
		UC_ConvertParameter(param,instance->mod3.vals,UGRP_RELFREQUENCY,RFRQ_FC);
		instance->frq2s=atof(param);
	}
	if(UC_GetParameterUnit(instance->mod3.vale,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		UC_ConvertParameter(param,instance->mod3.vale,UGRP_ABSFREQUENCY,AFRQ_HZ);
		instance->frq2e=(atof(param)*2.0)/(srcbuf->srat);
	}
	else {
		UC_ConvertParameter(param,instance->mod3.vale,UGRP_RELFREQUENCY,RFRQ_FC);
		instance->frq2e=atof(param);
	}
	instance->frq2d=instance->frq2e-instance->frq2s;
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod3.vals,"0.75",UGRP_RELFREQUENCY,RFRQ_FC);
	UC_ConvertParameter(instance->mod3.vale,"0.75",UGRP_RELFREQUENCY,RFRQ_FC);
