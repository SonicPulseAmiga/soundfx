/****h* Filter-FIRMatrix/lib-soundfx.c [4.2] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
...
*  FUNCTION
*    Filter-FIRMatrix Operator module for SoundFX
*  AUTHOR
...
*      hfrq=(double)(srat>>1);
*      frq=(hfrq/ranges);		// low pass
*      frq=hfrq-(hfrq/ranges);	// high pass
*
*******
*/

#define FILTER_FIRMATRIX_SOUNDFX_C
...
#define GADID_MATRIX	(GADID_AMPE+1)
#define GADIX_MATRIX	(GADIX_AMPE+1)

#define GADID_NR		(GADID_MATRIX+1)
#define GADIX_NR		(GADIX_MATRIX+1)

#define GADID_VAL		(GADID_NR+1)
#define GADIX_VAL		(GADIX_NR+1)

#define LAST_GADIX		(GADIX_VAL+1)
...
char matrixstr[64][14];			// wegen RefreshGUI
struct List MatrixList;
struct Node MatrixNodes[64];
UBYTE aktmat=0;
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	UWORD ys6=ys5+1+RunTime->scy[45+FRMTITLE_HEIGHT];
	...
	NewList(&MatrixList);			/* Liste bauen */
	for(j=0;j<64;j++) {
		sprintf(matrixstr[j],"%2d  %8.6lf",j,instance->p_matrix[j]);
		MatrixNodes[j].ln_Name=matrixstr[j];
		AddTail(&MatrixList,&MatrixNodes[j]);
	}
	...
	if(g=CreateContext(&modGList)) {
		...
		ng.ng_Height		=RunTime->scy[41];
		ng.ng_GadgetText	="Mat";
		ng.ng_GadgetID		=GADID_MATRIX;
		g=CreateGadget(LISTVIEW_KIND,g,&ng,GTLV_Labels,&MatrixList,GTLV_Selected,0,GTLV_Top,0,GTLV_ShowSelected,0l,TAG_DONE);
		modGadgets[GADIX_MATRIX]=g;
		...
		g=CreateGadget(INTEGER_KIND,g,&ng,GTIN_MaxChars,3,GTIN_Number,aktmat,TAG_DONE);
		modGadgets[GADIX_NR]=g;

		ng.ng_TopEdge		=ys5+RunTime->scy[FRMCONTENT_YOFFSET+12];
		ng.ng_GadgetText	="Val";
		ng.ng_GadgetID		=GADID_VAL;
		sprintf(param,"%8.6lf",instance->p_matrix[aktmat]);FilterString(param);
		g=CreateGadget(STRING_KIND,g,&ng,GTST_MaxChars,10,GTST_String,param,TAG_DONE);
		((struct StringInfo*)g->SpecialInfo)->Extension=RunTime->stringextend;
		...
		if(g) {
			...
			DrawTitle(modrp,"Matrix"		,xs1,ys5,STDFRM_WIDTH);
			...
			DrawGadWB(modrp,xs1,ys5,xh,RunTime->scy[45+FRMTITLE_HEIGHT]);			/* Matrix */
							...
							case IDCMP_GADGETUP:
								...
								&& !HandleCfgSel   (&cfgsel       ,modWin,modGadgets,GADID_CFGSEL,GADIX_CFGSEL,gnr,icode,isec,imic)) {
									switch(gnr) {
										...
										case GADID_MATRIX:
											aktmat=icode;
											GT_SetGadgetAttrs(modGadgets[GADIX_NR],modWin,0l,GTIN_Number,aktmat,TAG_DONE);
											sprintf(param,"%8.6lf",instance->p_matrix[aktmat]);FilterString(param);
											GT_SetGadgetAttrs(modGadgets[GADIX_VAL],modWin,0l,GTST_String,param,TAG_DONE);
											ActivateGadget(modGadgets[GADIX_VAL],modWin,0L);
											break;
										case GADID_NR:
											CheckGad_int(modGadgets[GADIX_NR],modWin,&val1,1,0,20);aktmat=(UBYTE)val1;
											sprintf(param,"%8.6lf",instance->p_matrix[aktmat]);FilterString(param);
											GT_SetGadgetAttrs(modGadgets[GADIX_VAL],modWin,0l,GTST_String,param,TAG_DONE);
											GT_SetGadgetAttrs(modGadgets[GADIX_MATRIX],modWin,0l,GTLV_Selected,aktmat,GTLV_Top,aktmat,TAG_DONE);
											ActivateGadget(modGadgets[GADIX_VAL],modWin,0L);
											break;
										case GADID_VAL:
											CheckGad_dbl(modGadgets[GADIX_VAL],modWin,&(instance->p_matrix[aktmat]),0,0,0);
											sprintf(matrixstr[aktmat],"%2d  %8.6lf",aktmat,instance->p_matrix[aktmat]);
											//if(aktmat<63) {
											//	aktmat++;
											//	GT_SetGadgetAttrs(modGadgets[GADIX_NR],modWin,0l,GTIN_Number,aktmat,TAG_DONE);
											//	sprintf(param,"%8.6lf",instance->p_matrix[aktmat]);FilterString(param);
											//	GT_SetGadgetAttrs(modGadgets[GADIX_VAL],modWin,0l,GTST_String,param,TAG_DONE);
											//	GT_SetGadgetAttrs(modGadgets[GADIX_MATRIX],modWin,0l,GTLV_Labels,&MatrixList,GTLV_Selected,aktmat,GTLV_Top,aktmat,TAG_DONE);
											//	ActivateGadget(modGadgets[GADIX_VAL],modWin,0L);
											//}
											//else
											GT_SetGadgetAttrs(modGadgets[GADIX_MATRIX],modWin,0l,GTLV_Labels,&MatrixList,GTLV_Selected,aktmat,GTLV_Top,aktmat,TAG_DONE);
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!stricmp(RexxPar0,"SFX_SetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Matrix"))		{ instance->p_matrix[atoi(RexxPar3)%64]=atof(RexxPar4);pf=TRUE; }
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Matrix"))		{ sprintf(ret,"%8.6lf",instance->p_matrix[atoi(RexxPar3)%64]); }
	}
...
void LoadSettings(struct Instance *instance,char *fname) {
	...
	for(i=0;i<64;i++) {
		sprintf(&strItemName[6],"%d",i);
		instance->p_matrix[i]=atof(CfgIO_ReadString(cfg,"General"	,strItemName,"1.0"));
	}
}
...
void SaveSettings(struct Instance *instance,char *fname) {
	...
	for(i=0;i<64;i++) {
		sprintf(&strItemName[6],"%d",i);
		CfgIO_WriteFloat	(cfg,"General"	,strItemName,&instance->p_matrix[i]);
	}
...
void SetDefaultSettings(struct Instance *instance) {
	...
	for(j=0;j<63;j++) instance->p_matrix[j]=1.0;
}
...
void RefreshGUI(struct Instance *instance,struct Gadget *modGadgets[],struct Window *modWin) {
	...
	GT_SetGadgetAttrs(modGadgets[GADIX_AMPS],modWin,0l,GTST_String,instance->p_amps,TAG_DONE);
	GT_SetGadgetAttrs(modGadgets[GADIX_AMPE],modWin,0l,GTST_String,instance->p_ampe,TAG_DONE);
	for(j=0;j<64;j++) sprintf(matrixstr[j],"%2d  %8.6lf",j,instance->p_matrix[j]);
	GT_SetGadgetAttrs(modGadgets[GADIX_MATRIX],modWin,0l,GTLV_Labels,&MatrixList,GTLV_Selected,aktmat,GTLV_Top,aktmat,TAG_DONE);
	sprintf(param,"%8.6lf",instance->p_matrix[aktmat]);FilterString(param);
	GT_SetGadgetAttrs(modGadgets[GADIX_VAL],modWin,0l,GTST_String,param,TAG_DONE);
}
