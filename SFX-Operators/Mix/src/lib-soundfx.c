...
*  FUNCTION
*    Mix Operator module for SoundFX
*  AUTHOR
...
*
*  IDEAS
*    ampf-Parameter (gleich die Mixfaktoren 'ampfen')
*******
*/

#define MIX_SOUNDFX_C
...
#define GADID_DELAY2	(GADID_MOD+MOD_IDCT)
#define GADIX_DELAY2	(GADIX_MOD+MOD_IXCT)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		if(g) {
			...
			DrawTitle(modrp,"Mixratio Source 1"	,xs1,ys3,STDFRM_WIDTH);
			DrawTitle(modrp,"Delay Source 2"	,xs1,ys4,STDFRM_WIDTH);
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	instance->mix1s=atof(param);
	UC_ConvertParameter(param,instance->mod.vale,UGRP_PROPOTION,PROP_FC);
	instance->mix1e=atof(param);
	instance->mix1d=instance->mix1e-instance->mix1s;

	if((pu=UC_GetParameterUnit(instance->p_delay2,UGRP_ABSTIME))!=INVALID_UNIT) {
		if(pu==ATME_SAMPLES) instance->delay2=atoi(instance->p_delay2);
		else { UC_ConvertParameter(param,instance->p_delay2,UGRP_ABSTIME,ATME_MS);instance->delay2=(ULONG)(atof(param)*((double)srcbuf2->srat/1000.0)); }
	}
	else { UC_ConvertParameter(param,instance->p_delay2,UGRP_RELTIME,RTME_FC);instance->delay2=(ULONG)((double)srcbuf2->slen*atof(param)); }
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!stricmp(RexxPar0,"SFX_SetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Delay2"))		{ strntcpy(instance->p_delay2,RexxPar3,PARBUFLEN);pf=TRUE; }
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Delay2"))		{ strntcpy(ret,instance->p_delay2,PARBUFLEN);/*pf=TRUE;*/ }
	}
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod.vals,"0.5",UGRP_PROPOTION,PROP_FC);
	UC_ConvertParameter(instance->mod.vale,"0.5",UGRP_PROPOTION,PROP_FC);
