...
*  FUNCTION
*    Fold Operator module for SoundFX
*  AUTHOR
...
*    21.Oct.1998	V 1.20	initial version
*  NOTES
*
*******
*/

#define FOLD_SOUNDFX_C
...
#define GADID_AMP		(GADID_MOD+MOD_IDCT)
#define GADIX_AMP		(GADIX_MOD+MOD_IXCT)

#define LAST_GADIX		(GADIX_AMP+1)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		g=CreateGadget(CHECKBOX_KIND,g,&ng,GTCB_Checked,instance->amp,GTCB_Scaled,TRUE,TAG_DONE);
							...
							case IDCMP_GADGETUP:
								...
								&& !HandleCfgSel   (&cfgsel      ,modWin,modGadgets,GADID_CFGSEL,GADIX_CFGSEL,gnr,icode,isec,imic)) {
									switch(gnr) {
										...
										case GADID_AMP:
											instance->amp=((modGadgets[GADIX_AMP]->Flags&GFLG_SELECTED)==GFLG_SELECTED);
											//MSG1("amp => %d",instance->amp);
											//MSG3("### => %d %d %d",modGadgets[GADIX_AMP]->Flags,GFLG_SELECTED,((modGadgets[GADIX_AMP]->Flags&GFLG_SELECTED)==GFLG_SELECTED));
											break;
									}
								}
								break;
					...
					if(srcsa && dstsa) {
						...
						if(instance->amp) {
							for(srcsa->seg=dstsa->seg=0;((srcsa->seg<srcsa->seganz) && (instance->curlen<instance->slen));srcsa->seg++,dstsa->seg++) {
								...
								process_amp((SFXCoreInstance *)instance,blen);
							...
							for(srcsa->seg=dstsa->seg=0;((srcsa->seg<srcsa->seganz) && (instance->curlen<instance->slen));srcsa->seg++,dstsa->seg++) {
								instance->sbuf=(SAMPLE *)srcsa->buffer1;SARead(srcsa,srcsa->blen);	// Pointer auf src-Block holen & nächstes Read starten
								instance->dbuf=(SAMPLE *)dstsa->buffer1;								// Pointer auf dst-Block
								process_noamp((SFXCoreInstance *)instance,blen);
								SAWrite(dstsa,dstsa->blen);									// akt Block schreiben
							}
						}
					}
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!stricmp(RexxPar0,"SFX_SetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Amp"))			{ instance->amp=atoi(RexxPar3)&0x1;pf=TRUE; }
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Amp"))			{ sprintf(ret,"%d",instance->amp);/*pf=TRUE;*/ }
	}
...
void LoadSettings(struct Instance *instance,char *fname) {
	...
	instance->amp=CfgIO_ReadBool	(cfg,"General"	,"Amp"	,TRUE);
}

void SaveSettings(struct Instance *instance,char *fname) {
	SaveModSettings(cfg,instance->moddesc);
	CfgIO_WriteBool				(cfg,"General"	,"Amp"	,instance->amp);
...
void RefreshGUI(struct Instance *instance,struct Gadget *modGadgets[],struct Window *modWin) {
	SetSource   (&(instance->src),modWin,modGadgets,GADIX_SRC);
	SetModulator(&(instance->mod),modWin,modGadgets,GADIX_MOD);
	GT_SetGadgetAttrs(modGadgets[GADIX_AMP],modWin,0l,GTCB_Checked,instance->amp,TAG_DONE);
}
