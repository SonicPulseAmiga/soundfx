/****h* QuantizeHoriz/lib-soundfx.c [4.2] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
...
*  FUNCTION
*    QuantizeHoriz Operator module for SoundFX
*  AUTHOR
...
#define QUANTIZEHORIZ_SOUNDFX_C
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		if(g) {
			...
			DrawTitle(modrp,"Quantization Range",xs1,ys3,STDFRM_WIDTH);
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	if(UC_GetParameterUnit(instance->mod2.vals,UGRP_ABSTIME)==ATME_SAMPLES) {
		instance->lens=atof(instance->mod2.vals);
	}
	else {
		UC_ConvertParameter(param,instance->mod2.vals,UGRP_ABSTIME,ATME_MS);
		instance->lens=atof(param)*((double)srcbuf->srat/1000.0);
	}
	if(UC_GetParameterUnit(instance->mod2.vale,UGRP_ABSTIME)==ATME_SAMPLES) {
		instance->lene=atof(instance->mod2.vale);
	}
	else {
		UC_ConvertParameter(param,instance->mod2.vale,UGRP_ABSTIME,ATME_MS);
		instance->lene=atof(param)*((double)srcbuf->srat/1000.0);
	}
	instance->lend=instance->lene-instance->lens;
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod1.vals,"0.8",UGRP_PROPOTION,PROP_FC);
	UC_ConvertParameter(instance->mod1.vale,"0.8",UGRP_PROPOTION,PROP_FC);
