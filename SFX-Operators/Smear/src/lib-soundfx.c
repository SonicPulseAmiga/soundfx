/****h* Smear/lib-soundfx.c [4.3] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
*    $VER: lib-soundfx.c 4.3 (23.11.03) © by Stefan Kost 1998-2003
*  FUNCTION
*    Smear Operator module for SoundFX
*  AUTHOR
...
#define GADID_INTER		(GADID_MOD2+MOD_IDCT)
#define GADIX_INTER		(GADIX_MOD2+MOD_IXCT)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		g=AddInterpolator(&(instance->int1),modGadgets,g,GADID_INTER,GADIX_INTER,xs1,ys4+RunTime->scy[FRMCONTENT_YOFFSET]);
		...
		if(g) {
			...
			RText(modrp,"Interp.",xs1+RunTime->scx[45],ys4+RunTime->scy[FRMTEXT_YPOS(1)]);

			DrawTitle(modrp,"Source"		,xs1,ys1,STDFRM_WIDTH);
			DrawTitle(modrp,"Effect"		,xs1,ys2,STDFRM_WIDTH);
			DrawTitle(modrp,"Smear Range"	,xs1,ys3,STDFRM_WIDTH);
			DrawTitle(modrp,"Miscellanous"	,xs1,ys4,STDFRM_WIDTH);
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	if(UC_GetParameterUnit(instance->mod2.vals,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		UC_ConvertParameter(param,instance->mod2.vals,UGRP_ABSFREQUENCY,AFRQ_HZ);if((rng=atof(param))<1.0) rng=1.0;
		instance->rngs=(double)srcbuf->srat/rng;
	}
	else {
		if((pu=UC_GetParameterUnit(instance->mod2.vals,UGRP_ABSTIME))!=INVALID_UNIT) {
			...
			else { UC_ConvertParameter(param,instance->mod2.vals,UGRP_ABSTIME,ATME_MS);instance->rngs=atof(param)*((double)srcbuf->srat/1000.0); }
		}
		else { UC_ConvertParameter(param,instance->mod2.vals,UGRP_RELTIME,RTME_FC);instance->rngs=(double)srcbuf->slen*atof(param); }
	}
	if(UC_GetParameterUnit(instance->mod2.vale,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		UC_ConvertParameter(param,instance->mod2.vale,UGRP_ABSFREQUENCY,AFRQ_HZ);if((rng=atof(param))<1.0) rng=1.0;
		instance->rnge=(double)srcbuf->srat/rng;
	}
	else {
		if((pu=UC_GetParameterUnit(instance->mod2.vale,UGRP_ABSTIME))!=INVALID_UNIT) {
			...
			else { UC_ConvertParameter(param,instance->mod2.vale,UGRP_ABSTIME,ATME_MS);instance->rnge=atof(param)*((double)srcbuf->srat/1000.0); }
		}
		else { UC_ConvertParameter(param,instance->mod2.vale,UGRP_RELTIME,RTME_FC);instance->rnge=(double)srcbuf->slen*atof(param); }
	}
	instance->rngd=instance->rnge-instance->rngs;
	instance->del=min(instance->rngs,instance->rnge);instance->del=fabs(instance->del);
	instance->off=(ULONG)instance->del;if(instance->del>(double)instance->off) instance->off++;		// lower delay value
	instance->del=(instance->off+fabs(instance->rngs));
	instance->dell=(instance->off+fabs(instance->rnge));
	if(instance->dell>instance->del) instance->del=instance->dell;

	MSG1("rngs : %8.5lf",instance->rngs);
	MSG1("rnge : %8.5lf",instance->rnge);
	MSG1("del  : %8.4lf",instance->del);
	MSG1("off  : %ld",instance->off);

	instance->del+=(ULONG)(instance->int1.range);
			...
			for(l=0;l<CH_COUNT(srcbuf->channels);l++) {
				if(IS_CHANNEL_ACTIVE(l,srcbuf->channelmask)) {
					...
					if(srcsa && dstsa) {
						...
						if(!ProWinAborted(instance->curlen)) {						// process rest (because of off delay)
							instance->rest=instance->off;
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf)    ParseInterpolatorGet(&(instance->int1), "I1",params,ret);
	}
