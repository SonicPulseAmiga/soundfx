/****h* Panorama-2Ch/lib-soundfx.c [4.2] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
...
*  FUNCTION
*    Panorama-2Ch Operator module for SoundFX
*  AUTHOR
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	if(srcbuf->channels==CH_MONO) {			// it's a mono sample
		if(!dstbuf) {
			if(dstbuf=NewSample()) {
				if(AllocSampleMem(dstbuf,srcbuf->slen,(CH_STEREO+1))) {
			...
			if(activate((SFXCoreInstance *)instance)) {
				if(IS_CHANNEL_ACTIVE(0,srcbuf->channelmask)) {
					...
					if(srcsa && dstsa0 && dstsa1) {
						...
						for(srcsa->seg=dstsa0->seg=dstsa1->seg=0;((srcsa->seg<srcsa->seganz) && (instance->curlen<instance->slen));srcsa->seg++,dstsa0->seg++,dstsa1->seg++) {
