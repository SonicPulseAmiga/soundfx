...
*  FUNCTION
*    NoiseGate Operator module for SoundFX
*  AUTHOR
...
*
*  IDEAS
*    use LPF which closes/opens when closing/opening the gate
*
*******
*/

#define NOISEGATE_SOUNDFX_C
...
#define GADID_ATTACK	(GADID_MOD1+MOD_IDCT)
#define GADIX_ATTACK	(GADIX_MOD1+MOD_IXCT)

#define GADID_SHAPE		(GADID_ATTACK+1)
#define GADIX_SHAPE		(GADIX_ATTACK+1)

#define LAST_GADIX		(GADIX_SHAPE+1)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		g=AddModulator(&(instance->mod),modGadgets,g,GADID_MOD1,GADIX_MOD1,xs1,ys2);
		...
		if(g) {
			...
			SetModulator(&(instance->mod),modWin,modGadgets,GADIX_MOD1);
					...
					while(imsg=GT_GetIMsg(modWin->UserPort)) {
						...
						switch(iclass) {
							case IDCMP_GADGETUP:
								...
								&& !HandleModulator(&(instance->mod),&(instance->src),0l,modWin,modGadgets,GADID_MOD1,GADIX_MOD1,gnr,icode)
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	UC_ConvertParameter(param,instance->mod.vals,UGRP_AMPLITUDE,AMP_LEVEL);
	instance->thrs=atof(param);
	UC_ConvertParameter(param,instance->mod.vale,UGRP_AMPLITUDE,AMP_LEVEL);
	instance->thre=atof(param);
	instance->thrd=instance->thre-instance->thrs;

	if(UC_GetParameterUnit(instance->p_attack,UGRP_ABSTIME)==ATME_SAMPLES) {
		instance->attack=atof(instance->p_attack);
	}
	else {
		UC_ConvertParameter(param,instance->p_attack,UGRP_ABSTIME,ATME_MS);
		instance->attack=atof(param)*((double)srcbuf->srat/1000.0);
	}
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Shape"))		{ strcpy(ret,ShapeLabels[instance->shape]); }
	}
...
void LoadSettings(struct Instance *instance,char *fname) {
	...
	strcpy(instance->p_attack,CfgIO_ReadString	(cfg,"General"	,"Attack"	,instance->p_attack));
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod.vals,"500 lv",UGRP_AMPLITUDE,AMP_LEVEL);
	UC_ConvertParameter(instance->mod.vale,"500 lv",UGRP_AMPLITUDE,AMP_LEVEL);
	instance->mod.bshape=BSHP_NONE;
	instance->mod.desc[0]='\0';

	UC_ConvertParameter(instance->p_attack,"0.5 ms",UGRP_ABSTIME,ATME_MS);
	instance->shape=CURVE_LINEAR;
}
