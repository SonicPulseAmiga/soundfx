/****h* Vocode-FFT/lib-soundfx.c [4.2] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
...
*  FUNCTION
*    Vocode-FFT Operator module for SoundFX
*  AUTHOR
...
*    04.Nov.1998	V 1.25	most recent version
...
#define GADID_WFKT		(GADID_STEPS+1)
#define GADIX_WFKT		(GADIX_STEPS+1)

#define GADID_AMPF		(GADID_WFKT+WFKT_IDCT)
#define GADIX_AMPF		(GADIX_WFKT+WFKT_IXCT)

#define GADID_EAMPF		(GADID_AMPF+1)
#define GADIX_EAMPF		(GADIX_AMPF+1)

#define GADID_SRC2INV	(GADID_EAMPF+1)
#define GADIX_SRC2INV	(GADIX_EAMPF+1)

#define GADID_EFCOEF	(GADID_SRC2INV+1)
#define GADIX_EFCOEF	(GADIX_SRC2INV+1)

#define LAST_GADIX		(GADIX_EFCOEF+1)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		ng.ng_LeftEdge		=xs1+RunTime->scx[214];
		ng.ng_GadgetText	="EAmpf";
		ng.ng_GadgetID		=GADID_EAMPF;
		g=CreateGadget(STRING_KIND,g,&ng,GTST_MaxChars,PARBUFLEN,GTST_String,instance->p_eampf,TAG_DONE);
		((struct StringInfo*)g->SpecialInfo)->Extension=RunTime->stringextend;
		modGadgets[GADIX_EAMPF]=g;
		...
		ng.ng_TextAttr		=RunTime->ScrFont;
		ng.ng_GadgetText	="Src2Inv";
		ng.ng_GadgetID		=GADID_SRC2INV;
		ng.ng_Flags			=PLACETEXT_LEFT;
		g=CreateGadget(CHECKBOX_KIND,g,&ng,GTCB_Checked,instance->src2inv,GTCB_Scaled,TRUE,TAG_DONE);
		modGadgets[GADIX_SRC2INV]=g;

		ng.ng_LeftEdge		=xs1+RunTime->scx[53];
		ng.ng_TopEdge		=ys4+RunTime->scy[FRMCONTENT_YOFFSET+36];
		ng.ng_Width			=RunTime->scx[60];
		ng.ng_GadgetText	="EFCoef";
		ng.ng_GadgetID		=GADID_EFCOEF;
		g=CreateGadget(STRING_KIND,g,&ng,GTST_MaxChars,PARBUFLEN,GTST_String,instance->efcoef,TAG_DONE);
		((struct StringInfo*)g->SpecialInfo)->Extension=RunTime->stringextend;
		modGadgets[GADIX_EFCOEF]=g;
		...
		if(g) {
			...
			DrawGadWB(modrp,xs1,ys4,xh,ANYFRM_HEIGHT(4));				/* Par */
								...
								&& !HandleCfgSel     (&cfgsel        ,modWin,modGadgets,GADID_CFGSEL,GADIX_CFGSEL,gnr,icode,isec,imic)) {
									switch(gnr) {
										...
										case GADID_AMPF:
											...
											break;
										case GADID_EAMPF:
											HandleParameter(instance->p_eampf,modWin,modGadgets[GADIX_EAMPF]);
											break;
										case GADID_SRC2INV:
											instance->src2inv=modGadgets[GADIX_SRC2INV]->Flags&GFLG_SELECTED;;
											break;
										case GADID_EFCOEF:
											HandleParameter(instance->efcoef,modWin,modGadgets[GADIX_EFCOEF]);
											break;
									}
								}
								break;
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	UC_ConvertParameter(param,instance->p_eampf,UGRP_AMPLITUDE,AMP_FC);
	instance->eampf=atof(param);

	UC_ConvertParameter(param,instance->efcoef,UGRP_PROPOTION,PROP_FC);
	instance->efc1=atof(param);
	instance->efc2=1.0-instance->efc1;
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!stricmp(RexxPar0,"SFX_SetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"EAmpf"))		{ strntcpy(instance->p_eampf,RexxPar3,PARBUFLEN);pf=TRUE; }
		...
		if(!pf && !stricmp(RexxPar2,"Src2Inv"))		{ instance->src2inv=atoi(RexxPar3)%2;pf=TRUE; }
		if(!pf && !stricmp(RexxPar2,"EFCoef"))		{ strntcpy(instance->efcoef,RexxPar3,PARBUFLEN);pf=TRUE; }
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"EAmpf"))		{ strntcpy(ret,instance->p_eampf,PARBUFLEN);pf=TRUE; }
		...
		if(!pf && !stricmp(RexxPar2,"Src2Inv"))		{ sprintf(ret,"%d",instance->src2inv);pf=TRUE; }
		if(!pf && !stricmp(RexxPar2,"EFCoef"))		{ strntcpy(ret,instance->efcoef,PARBUFLEN);/* pf=TRUE;*/ }
	}
...
void LoadSettings(struct Instance *instance,char *fname) {
	...
	strcpy(instance->p_eampf,	CfgIO_ReadString	(cfg,"General"	,"EAmpf"	,instance->p_eampf));
	strcpy(instance->efcoef,	CfgIO_ReadString	(cfg,"General"	,"EFCoef"	,instance->efcoef));
	instance->src2inv=		CfgIO_ReadBool		(cfg,"General"	,"Src2Inv"	,instance->src2inv);
...
void SaveSettings(struct Instance *instance,char *fname) {
	...
	CfgIO_WriteString		  (cfg,"General"	,"EAmpf"	,instance->p_eampf);
	CfgIO_WriteString		  (cfg,"General"	,"EFCoef"	,instance->efcoef);
	CfgIO_WriteBool			  (cfg,"General"	,"Src2Inv"	,instance->src2inv);
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->p_ampf,"4.0",UGRP_AMPLITUDE,AMP_FC);
	UC_ConvertParameter(instance->p_eampf,"10.0",UGRP_AMPLITUDE,AMP_FC);
	instance->bands=4;							/* Anzahl der Frequenzbänder 4=>64 */
	instance->steps=5;
	instance->src2inv=0;							/* don't invert modulator */
	UC_ConvertParameter(instance->efcoef,"0.95",UGRP_PROPOTION,PROP_FC);
}
...
void RefreshGUI(struct Instance *instance,struct Gadget *modGadgets[],struct Window *modWin) {
	...
	GT_SetGadgetAttrs(modGadgets[GADIX_EAMPF  ],modWin,0l,GTST_String,instance->p_eampf,TAG_DONE);
	GT_SetGadgetAttrs(modGadgets[GADIX_SRC2INV],modWin,0l,GTCB_Checked,instance->src2inv,TAG_DONE);
	GT_SetGadgetAttrs(modGadgets[GADIX_EFCOEF ],modWin,0l,GTST_String,instance->efcoef,TAG_DONE);
}
