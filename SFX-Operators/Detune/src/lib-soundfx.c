/****h* Detune/lib-soundfx.c [4.3] *
*
*  NAME
*    lib-soundfx.c
*  COPYRIGHT
*    $VER: lib-soundfx.c 4.3 (21.11.03) © by Stefan Kost 1998-2003
*  FUNCTION
*    Detune Operator module for SoundFX
*  AUTHOR
*    Stefan Kost			sk		kost@imn.htwk-leipzig.de
*  CREATION DATE
*    18.Jul.1998
*  MODIFICATION HISTORY
*    21.Nov.2003	V 4.3	most recent version
...
*  NOTES
*  IDEAS
*    * alternative fx-core implementation :
*      after every input-loop-block, write as much result-data we have
*      (currently we collect data)
*
*******
*/

#define DETUNE_SOUNDFX_C
...
#define GADID_INTER		(GADID_MOD+MOD_IDCT)
#define GADIX_INTER		(GADIX_MOD+MOD_IXCT)
...
void SAVEDS ASM *LIBSFXMod_ClientDataInit(REG(a0,RTime *RunTime_)) {
	...
	if(instance=(struct Instance *)AllocVec(sizeof(struct Instance),MEMF_ANY|MEMF_CLEAR)) {
		...
		SFXMod_ModDescInit(2,PD_INTERPOLATOR,"I1",&(instance->int1));
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		g=AddInterpolator(&(instance->int1),modGadgets,g,GADID_INTER,GADIX_INTER,xs1,ys3+RunTime->scy[FRMCONTENT_YOFFSET]);
		...
		if(g) {
			...
			RText(modrp,"Interp.",xs1+RunTime->scx[45],ys3+RunTime->scy[FRMTEXT_YPOS(1)]);

			DrawTitle(modrp,"Source"		,xs1,ys1,STDFRM_WIDTH);
			DrawTitle(modrp,"Detune"		,xs1,ys2,STDFRM_WIDTH);
			...
			DrawGadWB(modrp,xs1,ys3,xh,ANYFRM_HEIGHT(1));				/* Mode */
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	UC_ConvertParameter(param,instance->mod.vals,UGRP_RELFREQUENCY,RFRQ_FC);
	instance->factors=atof(param);
	UC_ConvertParameter(param,instance->mod.vale,UGRP_RELFREQUENCY,RFRQ_FC);
	instance->factore=atof(param);
	instance->factord=instance->factore-instance->factors;
		...
		if(activate((SFXCoreInstance *)instance)) {
			...
			for(l=0;l<CH_COUNT(srcbuf->channels);l++) {
				if(IS_CHANNEL_ACTIVE(l,srcbuf->channelmask)) {
					...
					srcsa=SAInit(srcbuf,l,rm_start,instance->slen,VM_MODE_READ |VM_MODE_SEQ|VM_MODE_BLK|VM_MODE_CBLK,VM_DEFAULT_BLKLEN);	// sequentiell Blöcke konstanter Größe lesen
					dstsa=SAInit(dstbuf,l,rm_start,instance->rlen,VM_MODE_WRITE|VM_MODE_SEQ|VM_MODE_BLK|VM_MODE_CBLK,VM_DEFAULT_BLKLEN);	// sequentiell Blöcke konstanter Größe schreiben
					if(srcsa && dstsa) {
						SARead =srcsa->safunc;
						...
						if(!ProWinAborted(instance->curlen)) {						// process rest (because of irange delay)
							while((instance->rptr<(double)instance->curlen) && (instance->curlen<=instance->slen)) {
					...
					if(rm_start+rm_len<srcbuf->slen) CopyDataCh(srcbuf,dstbuf,l,(rm_start+rm_len),(rm_start+instance->rlen),(srcbuf->slen-(rm_start+rm_len)));
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod.vals,"1.0",UGRP_RELFREQUENCY,RFRQ_FC);
	UC_ConvertParameter(instance->mod.vale,"1.0",UGRP_RELFREQUENCY,RFRQ_FC);
