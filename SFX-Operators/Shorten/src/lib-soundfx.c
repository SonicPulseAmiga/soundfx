...
*  NOTES
*    * was mache ich, wenn nicht alle Kanäle bearbeitet werden ?
*      -> einfach nur in aktiven Kanälen suchen, aber alle beschneiden ?
*******
*/
				...
				if(IS_CHANNEL_ACTIVE(l,srcbuf->channelmask)) {
					...
					if((srcsa=SAInit(srcbuf,l,rm_start,rm_len,VM_MODE_READ |VM_MODE_SEQ|VM_MODE_BLK|VM_MODE_CBLK,VM_DEFAULT_BLKLEN))) {	// sequentiell Blöcke konstanter Größe lesen
						...
						for(srcsa->seg=0;((srcsa->seg<srcsa->seganz) && (instance->curlen<instance->slen));srcsa->seg++) {
							...
							SARead(srcsa,srcsa->blen);	// Pointer auf src-Block holen & nächstes Read starten
					...
					if(AllocSampleMem(dstbuf,dstbuf->slen,CH_COUNT(srcbuf->channels))) {
						...
						for(l=0;l<CH_COUNT(srcbuf->channels);l++) {
							...
							if(IS_CHANNEL_ACTIVE(l,srcbuf->channelmask)) {							// ... der aktive ist
								...
								CopyDataCh(srcbuf,dstbuf,l,instance->rngs,rm_start,(instance->rnge-instance->rngs));		// Mittenbereich  übernehmen
								if(rm_start+rm_len<srcbuf->slen) CopyDataCh(srcbuf,dstbuf,l,(rm_start+rm_len),(rm_start+rm_len),(srcbuf->slen-(rm_start+rm_len)));
							}
							else CopyDataCh(srcbuf,dstbuf,l,0,0,dstbuf->slen);	// den kompletten kanal unverändert übernehmen (Länge ?)
						}
