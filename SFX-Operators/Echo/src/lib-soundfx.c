...
*  FUNCTION
*    Echo Operator module for SoundFX
*  AUTHOR
...
#define GADID_INTER		(GADID_MOD3+MOD_IDCT)
#define GADIX_INTER		(GADIX_MOD3+MOD_IXCT)

#define GADID_ANZ		(GADID_INTER+INT_IDCT)
#define GADIX_ANZ		(GADIX_INTER+INT_IXCT)

#define GADID_AMPF		(GADID_ANZ+1)
#define GADIX_AMPF		(GADIX_ANZ+1)
...
UBYTE SAVEDS ASM LIBSFXMod_Interface(REG(a0,struct Instance *instance)) {
	...
	if(g=CreateContext(&modGList)) {
		...
		ng.ng_LeftEdge		=xs1+RunTime->scx[155];
		...
		if(g) {
			...
			DrawTitle(modrp,"Delay"			,xs1,ys3,STDFRM_WIDTH);
			DrawTitle(modrp,"Amplitude"		,xs1,ys4,STDFRM_WIDTH);
...
UBYTE SAVEDS ASM LIBSFXMod_Process(REG(a0,struct Instance *instance),REG(a1,ProcessData *pdata)) {
	...
	if(UC_GetParameterUnit(instance->mod2.vals,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		...
		if((instance->dels=atof(param))<1.0) instance->dels=1.0;
		instance->dels=(double)srcbuf->srat/instance->dels;
	}
	else {
		if((pu=UC_GetParameterUnit(instance->mod2.vals,UGRP_ABSTIME))!=INVALID_UNIT) {
			if(pu==ATME_SAMPLES) instance->dels=atof(instance->mod2.vals);
			else {
				UC_ConvertParameter(param,instance->mod2.vals,UGRP_ABSTIME,ATME_MS);
				instance->dels=atof(param)*((double)srcbuf->srat/1000.0);
			}
		}
		else {
			UC_ConvertParameter(param,instance->mod2.vals,UGRP_RELTIME,RTME_FC);
			instance->dels=(double)srcbuf->slen*atof(param);
		}
	}
	...
	if(UC_GetParameterUnit(instance->mod2.vale,UGRP_ABSFREQUENCY)!=INVALID_UNIT) {
		...
		if((instance->dele=atof(param))<1.0) instance->dele=1.0;
		instance->dele=(double)srcbuf->srat/instance->dele;
	}
	else {
		if((pu=UC_GetParameterUnit(instance->mod2.vale,UGRP_ABSTIME))!=INVALID_UNIT) {
			if(pu==ATME_SAMPLES) instance->dele=atof(instance->mod2.vale);
			else {
				UC_ConvertParameter(param,instance->mod2.vale,UGRP_ABSTIME,ATME_MS);
				instance->dele=atof(param)*((double)srcbuf->srat/1000.0);
			}
		}
		else {
			UC_ConvertParameter(param,instance->mod2.vale,UGRP_RELTIME,RTME_FC);
			instance->dele=(double)srcbuf->slen*atof(param);
		}
	}
	...
	if(instance->anz<1) instance->anz=1;
	MSG1("Number : %d",instance->anz);
...
void SAVEDS ASM LIBSFXMod_HandleARexxCmd(REG(a0,struct Instance *instance),REG(a1,char *params),REG(a2,char *ret)) {
	...
	if(!stricmp(RexxPar0,"SFX_SetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Number"))		{ strntcpy(instance->p_anz  ,RexxPar3,PARBUFLEN);pf=TRUE; }
	...
	if(!cf && !stricmp(RexxPar0,"SFX_GetOperatorParam")) {
		...
		if(!pf && !stricmp(RexxPar2,"Number"))		{ strntcpy(ret,instance->p_anz  ,PARBUFLEN);pf=TRUE; }
...
void LoadSettings(struct Instance *instance,char *fname) {
	...
	strcpy(instance->p_ampf,CfgIO_ReadString			(cfg,"General"	,"Ampf"		,"0.7"));
	strcpy(instance->p_anz ,CfgIO_ReadString			(cfg,"General"	,"Number"	,"5"));
}

void SaveSettings(struct Instance *instance,char *fname) {
	SaveModSettings(cfg,instance->moddesc);
	CfgIO_WriteString		  (cfg,"General"	,"Ampf"		,instance->p_ampf);
	CfgIO_WriteString		  (cfg,"General"	,"Number"	,instance->p_anz);
...
void SetDefaultSettings(struct Instance *instance) {
	...
	UC_ConvertParameter(instance->mod2.vals,"10 ms",UGRP_ABSTIME,ATME_MS);
	UC_ConvertParameter(instance->mod2.vale,"1 s",UGRP_ABSTIME,ATME_MS);
	...
	UC_ConvertParameter(instance->mod3.vals,"0.7" ,UGRP_AMPLITUDE,AMP_FC);
	UC_ConvertParameter(instance->mod3.vale,"0.01",UGRP_AMPLITUDE,AMP_FC);
	...
	UC_ConvertParameter(instance->p_anz,"5" ,UGRP_COUNT,COUN_ABS);
	UC_ConvertParameter(instance->p_ampf,"0.7" ,UGRP_AMPLITUDE,AMP_FC);
}
